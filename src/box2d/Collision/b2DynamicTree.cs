...
//        private uint m_path;
        ...
        // Stack used for queries/raycast; resized as needed to avoid overflow.
        private static int[] _stack = new int[1024];

        private static int[] EnsureStackCapacity(int required)
        {
            if (_stack.Length < required)
            {
                _stack = new int[required];
            }
            return _stack;
        }
...
//            m_path = 0;
        ...
        {
            int stackCount = 0;
            var stack = EnsureStackCapacity(m_nodeCapacity);
